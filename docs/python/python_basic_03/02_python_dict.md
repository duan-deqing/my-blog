...
字典推导式还可以用于将列表转换为字典，例如将列表中的元素作为键，索引作为值。
...
`defaultdict` 是 `collections` 模块中的一个类，它继承自 `dict` 类，并提供了默认值的特性。
...
`Counter` 是 `collections` 模块中的一个类，它用于统计可迭代对象中元素的个数。
...
字典是一种非常高效的数据结构，它的时间复杂度为 O(1)，即无论字典的大小如何，查找、插入和删除操作的时间都是常数时间。
